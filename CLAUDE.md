...
Claude 運用ドキュメントは以下の **3つの置き場** に集約する。これ以外の置き場（過去に存在した `repo/` や `learning/` 等）は作らない。新しい文書を書くときは、必ずこの3分類のどれかに振り分ける。`roadmap/` だけは「進捗の管制盤」として別扱い（後述）。

| 置き場 | 役割 | 命名 |
|---|---|---|
| `docs/` | 永続ドキュメント（タスク非依存・常に最新） | ファイル名は内容を表す任意名 |
| `steering/` | タスク別設計書 | `YYYYMMDD-タイトル/design.md` |
| `docs/knowledge/` | ユーザー専用の学習ノート | `YYYY-MM-DD/{topic}.md` |
| `roadmap/` | プロジェクト全体の進捗管制盤（特別扱い） | `roadmap.md` |

### `roadmap/`（進捗の管制盤・常時監視）

プロジェクト全体の実装計画と進捗を管理する**生きたドキュメント**。参照資料ではなく「今どこをやっているか」を示す管制盤なので、3バケツとは分けて専用フォルダに置く。

```
roadmap/
  roadmap.md    # フェーズ別計画 + 📍現在地ブロック
```

- 冒頭の `<!-- CURRENT-STATUS:START -->` 〜 `<!-- CURRENT-STATUS:END -->` で囲まれた **📍現在地ブロック**は、**SessionStart フックで毎セッション開始時に自動表示**される（`.claude/settings.local.json`）。
- **タスクの着手・完了時には、まず `roadmap/roadmap.md` の📍現在地ブロックと該当フェーズの進捗マーカー（✅/🔶/❌）を更新する。** これを進捗メンテの起点とする。

### `docs/`（永続ドキュメント）

プロジェクト全体を通じて常に最新を維持するドキュメントを置く。DBスキーマ・ER図・インフラ構成・横断的な設計判断など、特定のタスクに紐づかない情報。

```
docs/
  db-schema.md               # テーブル定義・ER図・接続情報
  development-guidelines.md   # コミット規約・コーディング規約
  api-response-design.md      # APIレスポンスの横断設計
  tech-debt.md                # 技術的負債の記録
```

### `steering/`（タスク別設計書）

タスクごとの設計書を `YYYYMMDD-タイトル/design.md` の形式で保存する。実装の意思決定・API仕様・シーケンス図など、そのタスクに紐づく情報。完了済みタスクの計画書もここに蓄積する。
...
```
docs/knowledge/
  _TEMPLATE.md          # Zenn流の記事テンプレート（執筆時はこれをベースにする）
  2026-06-05/
...
- ナレッジを書くときは `docs/knowledge/_TEMPLATE.md`（Zenn流の技術記事テンプレート）をベースにする。
- 会話内容からナレッジを起こす場合は `/write-knowledge` スキルを使う（テンプレ準拠で `docs/knowledge/<日付>/` に生成）。

**重要:**

- 陳腐化しても削除・移動しない。すべて蓄積し続ける
- **Claudeはこのフォルダを質問回答・実装の根拠として参照しない。**ユーザーから明示的に「ナレッジに追加して」「ナレッジを更新して」と依頼されたときのみ書き込む
- ファイル冒頭には最小限のメタ情報（書いた日・関連コード）を記載する（テンプレの frontmatter がこれを満たす）
